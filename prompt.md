はい、承知いたしました。`prompt.txt` の内容をマークダウン形式（.md）に変換します。
...
-----
  ...
  * このMCPサーバーは「遠慮したラップトップ和歌集」専用のものです。ただし、「遠慮したラップトップ和歌集」という名前は例えば「ラップトップ和歌集」という名前に変更する可能性もあります。その名前の変更はユーザーに完全に周知されます。
